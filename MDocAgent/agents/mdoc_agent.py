...
class MDocAgent(MultiAgentSystem):
    ...
    def predict(self, question, texts, images):
        ...
        general_response, messages, usage, elapsed = general_agent.predict(question, texts, images, with_sys_prompt=True)
        ...
        text_response, messages, usage, elapsed = text_agent.predict(question + relect_prompt +text_reflection, texts = texts, images = None, with_sys_prompt=True)
        all_messages += "Text Agent:\n" + text_response + "\n"
        image_response, messages, usage, elapsed = image_agent.predict(question + relect_prompt +image_reflection, texts = None, images = images, with_sys_prompt=True)
