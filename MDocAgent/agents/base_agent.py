...
class Agent:
    ...
    def _predict(self, question, texts=None, images=None, add_to_message = False):
        ...
        if not self.config.agent.use_image:
            images = None
        result = self.model.predict(question, texts, images, self.messages)
        if isinstance(result, tuple) and len(result) == 4:
            generated_ans, messages, usage, elapsed = result
            print(f"[AgentSummary] agent={self.__class__.__name__}, usage={usage}, elapsed={elapsed:.2f}s, answer={generated_ans[:100]}...")
        else:
            generated_ans, messages = result
            usage, elapsed = None, None
        if add_to_message:
            self.messages = messages
        return generated_ans, messages, usage, elapsed
    ...
    def predict(self, question, texts=None, images=None, with_sys_prompt=True):
        ...
        generated_ans, messages, usage, elapsed = self._predict(question, texts, images, add_to_message = True)
        total_elapsed = time.time() - start_time
        print(f"[AgentTotal] agent={self.__class__.__name__}, total_elapsed={total_elapsed:.2f}s, answer={generated_ans[:100]}...")
        return generated_ans, messages, usage, total_elapsed
    ...
    def self_reflect(self, prompt=None, add_to_message = True):
        ...
        generated_ans, messages, usage, elapsed = self._predict(question = self_reflect_prompt)
        if add_to_message:
            self.messages = messages
