...
class MultiAgentSystem:
    ...
    def sum(self, sum_question):
        ans, all_messages, usage, elapsed = self.sum_agent.predict(sum_question)
    ...
    def predict_dataset(self, dataset:BaseDataset, resume_path = None):
        ...
        for sample in tqdm(samples):
            ...
            try:
                ...
                print(f"RuntimeError for sample {sample_no}: {e}")
                if "out of memory" in str(e):
                    torch.cuda.empty_cache()
                final_ans, final_messages = None, None
            except Exception as e:
                print(f"Unexpected error for sample {sample_no}: {e}")
